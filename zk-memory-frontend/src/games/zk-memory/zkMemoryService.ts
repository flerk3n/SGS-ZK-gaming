import { Client as ZkMemoryClient, type Game } from './bindings';
...
/**
 * Service for interacting with the ZkMemory game contract
 */
export class ZkMemoryService {
  private baseClient: ZkMemoryClient;
  private contractId: string;

  constructor(contractId: string) {
    this.contractId = contractId;
    // Base client for read-only operations
    this.baseClient = new ZkMemoryClient({
  ...
  private createSigningClient(
    ...
    return new ZkMemoryClient(options);
  }
  ...
  async prepareStartGame(
    ...
    const buildClient = new ZkMemoryClient({
      contractId: this.contractId,
      networkPassphrase: NETWORK_PASSPHRASE,
      rpcUrl: RPC_URL,
  ...
  async importAndSignAuthEntry(
    ...
    const tx = await buildClient.start_game({
      ...
      deck_commitment: deckCommitment,       // Provided by Player 2
    }, DEFAULT_METHOD_OPTIONS);
    // NOTE: Contract methods automatically simulate - no need to call tx.simulate() again!
  ...
  async finalizeStartGame(
    ...
    console.log('[finalizeStartGame] Starting finalization...');
    ...
    console.log('[finalizeStartGame] Re-simulating transaction...');
    await tx.simulate();
    console.log('[finalizeStartGame] Simulation successful');
    ...
    console.log('[finalizeStartGame] Transaction submitted successfully!');
    return sentTx.result;
  }
   ...
   * Returns session ID, player addresses, points, deck commitment, and transaction source
  ...
  parseTransactionXDR(xdr: string): {
    ...
    // Extract deck_commitment (BytesN<32>)
    const deckCommitmentScVal = args[5];
    const deckCommitment = Buffer.from(deckCommitmentScVal.bytes());
  ...
  async flipCard(
    ...
    const tx = await client.flip_card({
      session_id: sessionId,
      player,
      position,
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes('Transaction failed!')) {
        throw new Error('Transaction failed - check if it\'s your turn and the card is not already matched');
      }
      throw err;
    }
  }
...
// Example: const zkMemoryService = new ZkMemoryService(contractId);
